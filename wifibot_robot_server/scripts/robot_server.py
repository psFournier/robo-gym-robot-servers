...
from wifibot_robot_server.ros_bridge import RosBridge
...
def serve():
    ...
    server.start()
    if real_robot:
        rospy.loginfo('Wifibot Real Robot Server started at ' + repr(server_port))
    else:
        rospy.loginfo('Wifibot Sim Robot Server started at ' + repr(server_port))
    rospy.spin()
